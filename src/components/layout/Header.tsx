    ...
    >
      <div className="container-content h-full flex items-center justify-between">
        <Link to="/" className="flex items-center gap-[7px] font-extrabold text-[15px] tracking-[-0.3px] text-t-primary">
          <div className="w-[26px] h-[26px] rounded-[7px] flex items-center justify-center text-[13px] text-white" style={{ background: "linear-gradient(135deg, #2563EB, #06B6D4)" }}>✦</div>
          LarkConsult
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a href="https://zalo.me/0905652628" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm font-medium text-t-tertiary px-2.5 py-1.5 rounded-md border border-white/5 hover:text-t-secondary hover:border-white/10 transition-all">
        ...
        <div className="flex md:hidden items-center gap-2">
          <a href={CTA_LINK} target="_blank" rel="noopener noreferrer" className="btn-primary !py-2 !px-4 !text-xs">Tư vấn</a>
      ...
      {mobileOpen && (
        <div className="md:hidden animate-fade-in" style={{ background: "rgba(6,13,24,0.95)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div className="container-content py-4 flex flex-col gap-3">
            {navItems.map((item) => (
              <a key={item.label} href={item.href} onClick={() => setMobileOpen(false)} className="text-sm font-medium text-t-primary py-2">{item.label}</a>
            ))}
            <Link to="/case-studies" onClick={() => setMobileOpen(false)} className="text-sm font-medium text-t-primary py-2">Case Study</Link>
            <a href="https://zalo.me/0905652628" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm font-medium text-t-secondary py-2">Zalo: 0905 652 628</a>
          </div>
        </div>
      )}
    </nav>
